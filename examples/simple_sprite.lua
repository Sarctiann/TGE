...
game.on_event = function(e)
	...
	elseif e.char == "G" then
		cancel = game.create_sequence({ 7, 9, 5 }, {
			{ ui_element = tank, action = 3, when = game.sf, data = { pos = 1, orientation = 1 } },
			{ ui_element = tank, action = 3, when = game.sf, data = { pos = 2, orientation = 2 } },
			{ ui_element = tank, action = 3, when = game.sf, data = { pos = 3, orientation = 3 } },
			{ ui_element = tank, action = 3, when = game.sf, data = { pos = 4, orientation = 4 } },
		}, nil, true)
	elseif e.char == "g" and cancel then
		cancel()
